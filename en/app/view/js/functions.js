...
function gridPostLoad(arr,input){
	...
	for(var i=0;i<arr.length;i++){
		...
		if (arr[i].estatus_publicacion!=null) {
			...
			var publicar=$("<input type='button'></input>").addClass("btn btn-primary btn-block").val("Publicar").attr("data-toggle","modal").attr("data-target","#postModal").attr("onclick","$('#actualizar').hide();$('#publicar').show();$('#id_marca').val("+arr[i].id_marca+");reset();$('#sku').val('"+arr[i].sku+"');$('.check').hide()");
